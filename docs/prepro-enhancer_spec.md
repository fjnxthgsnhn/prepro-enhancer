...
TSVおよびJSON内では原則として相対パスを使用する。

```text
media/images/cut001.jpg
media/audio/cut001.wav
media/references/lab_ref_001.jpg
```
...
`cutlist.tsv` は構造・順序・代表素材パスを持つ軽量なカット表とする。

LLMは `cutlist.tsv` を直接編集できる。ただし、複数テイク、OK/NG属性、生成素材履歴、プロンプト本文の正本はJSON側で管理する。

`image_prompt` および `video_prompt` は段階的に非正本化し、LLM互換出力や簡易編集のための展開列として扱う。プロンプト本文の正本は `prompts.json` とする。

### 5.2 assets.json

`assets.json` は、静止画または動画を生成するためのリファレンス静止画の正本とする。

`assets.json` で管理する対象は以下。

- 生成時に参照する静止画
- キャラクター、背景、美術、構図、質感などの参照素材
- プロンプトまたは生成素材から参照される素材ID

`assets.json` は生成用リファレンス専用とする。cutに割り当てる完成素材は `generate.json` で管理する。

### 5.3 generate.json

`generate.json` は、カットリストに紐づいた生成素材および割当素材の正本とする。

`generate.json` で管理する対象は以下。

- cutに割り当てる静止画ファイル
- cutに割り当てる音声ファイル
- 将来拡張としてcutに割り当てる動画ファイル
- image / audio / video の複数テイク
- OK / NG / 保留などのテイク属性
- 生成元プロンプトおよび参照素材との紐づけ

`generate.json` で `isActive` が `true` の素材は、`cutlist.tsv` の代表パスへ同期する。

### 5.4 prompts.json

`prompts.json` は、scene / multicut / cut のプロンプト本文の正本とする。

`prompts.json` で管理する対象は以下。
...
有効プロンプトは従来通り、階層に沿って以下を合成する。

```text
scene prompt + multicut prompt + cut prompt
```
...
MVPでは、構造と代表値は `cutlist.tsv` から復元できる設計とする。複数テイク、プロンプト履歴、参照素材の詳細はJSONから復元する。
...
`image`、`audio_file`、`video_file` は採用中または代表素材のパスのみを保持する。複数テイクやOK/NG属性は `generate.json` を正本とする。

`image_prompt`、`video_prompt` はLLM互換性のために保持できる展開列とし、プロンプト本文の正本は `prompts.json` とする。
...
| image | 任意 | 採用中または代表の静止画ファイルパス |
| audio_file | 任意 | 採用中または代表の音声ファイルパス |
| video_file | 任意 | 将来拡張用。採用中または代表の動画ファイルパス。MVPではプレビューおよびXML出力対象外 |
| image_prompt | 任意 | LLM互換出力用の静止画生成プロンプト展開列。正本は `prompts.json` |
| video_prompt | 任意 | LLM互換出力用の動画生成プロンプト展開列。正本は `prompts.json` |
| scene | 任意 | シーン名または場所名 |
| subject | 任意 | 被写体 |
| composition | 任意 | 構図 |
| action | 任意 | 動作 |
...
`id` は内部同期の基準であり、Table ViewおよびDetail Panelでは先頭に表示する。UI上では打ち換え不可とする。

`assets.json`、`generate.json`、`prompts.json` では種類ごとの一意IDを使用する。

```text
reference asset: asset_001, asset_002
generated media: gen_001, gen_002
prompt:          prompt_001, prompt_002
```

cutとの紐づけは必ず `cutId` または `ownerId` で行う。ファイルパス文字列だけを同期キーにしない。
...
cut	ct001	mc001	1	顕微鏡の寄り	3s	draft	media/images/cut001.jpg	media/audio/cut001.wav		顕微鏡レンズの超クローズアップ、青白い研究室照明、浅い被写界深度	顕微鏡レンズへゆっくりドリーイン。微細な反射が揺れ、緊張感を高める。	研究所	顕微鏡	超寄り	レンズが光る	ゆっくりドリーイン	低い機械音	導入カット
cut	ct002	mc001	2	山本の目元	3s	review	media/images/cut002.jpg			研究員の目元アップ、真剣な表情、青白い照明	山本が顕微鏡を覗き込み、わずかに眉を寄せる。カメラは微細にプッシュイン。	研究所	山本	目元アップ	違和感に気づく	微細なプッシュイン	息を呑む音	表情重視
```
...
LLMは `cutlist.tsv` を直接編集できる。

アプリはLLM編集後のTSVを読み込み、階層・ID・順序・必須カラムの整合性を検証する。

プロンプト本文の正本は `prompts.json`、複数テイクおよびOK/NG属性の正本は `generate.json` とする。LLMに渡すTSVでは、必要に応じてJSON内のactiveなプロンプトと代表素材を展開する。
...
`image_prompt` と `video_prompt` は、LLM編集時には編集可能な展開列として扱う。再インポート時には `prompts.json` のactiveプロンプトへ反映する。
...
`image`、`audio_file`、`video_file` は代表素材パスであり、直接編集された場合は `generate.json` の対応アイテムを作成またはactive化する。
...
```tsv
id	title	duration	status	image_prompt	video_prompt	note
```

LLM向けTSV出力では、`prompts.json` のactiveなプロンプトを `image_prompt` / `video_prompt` に展開し、`generate.json` のactiveな素材を `image` / `audio_file` / `video_file` に展開する。

LLM編集後のTSVを再インポートする場合、以下の同期を行う。

1. TSVの代表素材パスが変更されている場合、`generate.json` の同種アイテムを作成またはactive化する
2. TSVの `image_prompt` / `video_prompt` が変更されている場合、`prompts.json` のactiveプロンプトを更新する
3. 既存のOK/NGテイク、非active素材、参照素材IDは削除しない
...
`assets.json` は生成用リファレンス静止画を管理する。

例：

```json
{
  "format": "LocalCutBoardAssets",
  "formatVersion": "1.0.0",
  "assets": [
    {
      "id": "asset_001",
      "type": "reference_image",
      "path": "media/references/lab_ref_001.jpg",
      "title": "研究所照明リファレンス",
      "tags": ["background", "lighting"],
      "note": "青白い研究所照明の参照",
      "createdAt": "2026-05-22T15:30:00+09:00"
    }
  ]
}
```
...
`generate.json` はcutに紐づく image / audio / video の生成素材および割当素材を管理する。

例：

```json
{
  "format": "LocalCutBoardGeneratedMedia",
  "formatVersion": "1.0.0",
  "items": [
    {
      "id": "gen_001",
      "cutId": "ct001",
      "type": "image",
      "path": "media/images/cut001_takeA.jpg",
      "takeName": "takeA",
      "status": "ok",
      "isActive": true,
      "note": "採用候補",
...
| type | 内容 |
|---|---|
| image | cutに割り当てる静止画ファイル |
| audio | cutに割り当てる音声ファイル |
| video | cutに割り当てる動画ファイル。MVPではプレビューおよびXML出力対象外 |
...
同一 `cutId` かつ同一 `type` で `isActive: true` にできる素材は最大1件とする。

### 10.3 prompts.json

`prompts.json` はscene / multicut / cutのプロンプトを管理する。

例：

```json
{
  "format": "LocalCutBoardPrompts",
  "formatVersion": "1.0.0",
  "prompts": [
    {
      "id": "prompt_001",
      ...
      "ownerType": "cut",
      "kind": "image",
      "text": "顕微鏡レンズの超クローズアップ。青白い研究室照明。",
      "status": "active",
      "isActive": true,
      "linkedAssetIds": ["asset_001"],
      "note": "asset_001を照明参照として使用",
      "createdAt": "2026-05-22T15:30:00+09:00",
      "updatedAt": "2026-05-22T15:30:00+09:00"
    }
  ]
}
```

`ownerType` は `scene` / `multicut` / `cut` を許可する。`kind` は `image` / `video` を許可する。

同一 `ownerId` かつ同一 `kind` で `isActive: true` にできるプロンプトは最大1件とする。

### 10.4 JSON同期ルール

`generate.json` のactive素材が変わった場合、保存時に `cutlist.tsv` の代表パスを更新する。

対応関係は以下とする。

| generate.json type | cutlist.tsv column |
|---|---|
| image | image |
| audio | audio_file |
| video | video_file |

`cutlist.tsv` の代表パスを直接編集した場合、同一 `cutId` / `type` / `path` の `generate.json` アイテムをactive化する。存在しない場合は新規作成する。

`prompts.json` のactiveプロンプトが変わった場合、LLM互換TSV出力時に `image_prompt` / `video_prompt` へ展開する。

### 10.5 削除ルール

cutを削除した場合、関連する `generate.json` および `prompts.json` の項目は論理削除または孤立項目として警告する。

アプリは参照先のメディアファイル本体を自動削除しない。
...
将来拡張用として、`cutlist.tsv` の `video_file` と `generate.json` の `type: "video"` は定義する。ただしMVPでは動画プレビュー、動画トリム、Premiere XMLへの動画クリップ出力は行わない。
...
各行は `id` を正式な識別子として表示する。`id` は自動発行され、UI上では打ち換え不可とする。
...
- imageが存在するか
- audio_fileが存在するか
- generate.jsonのactiveなimage / audio / videoの参照先が存在するか
- assets.jsonのリファレンス静止画が存在するか
- サポート形式か
- 相対パスとして解決できるか
...
- `assets.json`、`generate.json`、`prompts.json` が存在するか。存在しない場合は空ファイルまたはTSV由来の初期データを生成できるか
- `generate.json` の `cutId` が存在するcutを参照しているか
- `prompts.json` の `ownerId` が存在するscene / multicut / cutを参照しているか
- `sourcePromptId` が存在するプロンプトを参照しているか
- `referenceAssetIds` および `linkedAssetIds` が存在するassetを参照しているか
- 同一cut、同一typeのactive生成素材が複数存在しないか
- 同一owner、同一kindのactiveプロンプトが複数存在しないか
...
- Missing Mediaの再リンク
- 空durationへのデフォルト値設定
- 欠落しているassets.json / generate.json / prompts.jsonの初期生成
- TSVのimage / audio_file / video_fileからgenerate.jsonのactive素材を作成
- TSVのimage_prompt / video_promptからprompts.jsonのactiveプロンプトを作成
...
プロンプト:
  prompts.jsonでimage_prompt / video_promptを管理
  cutlist.tsvのimage_prompt / video_promptはLLM互換展開列

LLM連携:
  LLMがcutlist.tsvを直接編集
  activeなプロンプトと代表素材をTSVへ展開

素材管理:
  assets.json = 生成用リファレンス静止画
  generate.json = cutに紐づくimage / audio / video素材とOK/NGテイク
  cutlist.tsv = 採用中または代表素材パス
...
本仕様では、動画ファイルの実利用をMVPから外すことで実装リスクを抑えつつ、LLMによるTSV編集、静止画確認、音声確認、プロンプト管理、生成素材のOK/NGテイク管理、グループ構造、Premiere連携を優先する。
